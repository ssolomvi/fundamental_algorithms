...
int main(int argc, char** argv)
{
	...
	char* lex1 = NULL, *lex2 = NULL, *lex3 = NULL, *new_str = NULL, ch, _ch = 0;
	...
	while (!feof(file_r)) {
		...
		// ! below !
		// HEAP CORRUPTION DETECTED: after Normal block (#79)
		// CRT detectd that the application wrote to memory after end of heap buffer
		free(lex1);
		free(lex2);
		free(lex3);
	}
...
read_lex_statuses read_lex(char** lex, int* size, FILE* file_r, char ch)
{
	...
	if (!(tmp = (char*)realloc((*lex), (*size) * sizeof(char)))) {
		free(*lex);
		return read_lex_reallocation_error;
	}
	*lex = tmp;
